...
(function () {
  ...
  // 7. PASSOS: fonte ÚNICA do fluxo de onboarding.
  //    A ordem do array É a ordem do fluxo. Para mudar o fluxo (adicionar, remover
  ...
  //    O rótulo é curto de propósito: no desktop ele mora embaixo de um círculo
  //    de 30px e sete deles dividem a mesma largura.
  AppCore.PASSOS = [
  ...
  // Escreve um número NUM CAMPO de dinheiro já no formato pt-BR completo,
  // com separador de milhar: 500000 vira "500.000,00".
  //
  // Existe porque o código escrevia `n.toFixed(2).replace('.',',')` em nove
  // lugares — idioma que produz "500000,00", sem os pontos de milhar, e que
  // some com o agrupamento assim que a tela repõe um valor. Um único ponto de
  // escrita mantém o que o usuário digita e o que o app repõe com a MESMA cara.
  AppCore.escreverMoeda = function (alvo, valor) {
    var el = typeof alvo === 'string' ? document.getElementById(alvo) : alvo;
    if (!el) return;
    if (valor === null || valor === undefined || valor === '') { el.value = ''; return; }
    el.value = AppCore.parseNum(valor)
  ...
  //
  // Ao ligar, JÁ NORMALIZA o valor que estiver no campo: campo preenchido pelo
  // HTML ou por JS antes desta chamada também sai agrupado, sem depender de o
  // usuário digitar nele.
  AppCore.ligarMoeda = function (raiz) {
    ...
    for (var i = 0; i < campos.length; i++) {
      (function (el) {
        ...
        if (el.value.trim()) AppCore.normalizarMoeda(el);
      })(campos[i]);
    }
  };
  ...
  // 12. Campos numéricos: sem setas e imunes à roda do mouse.
  //
  //     Duas fontes de erro silencioso no mesmo controle:
  //       1. As setinhas do spinner são alvo pequeno, encostado no campo, e
  //          alteram um valor financeiro em um clique acidental.
  //       2. Rolar a página com o ponteiro sobre um campo numérico FOCADO faz
  //          o navegador somar/subtrair no lugar de rolar. O usuário vê o
  //          número mudar sem entender por quê.
  //
  //     A seta some no CSS (shared.css). Aqui trata-se a roda: em vez de
  //     bloquear o scroll — o que prenderia a página —, o campo PERDE O FOCO.
  //     A página rola normalmente e o valor não muda.
  //
  //     Um único listener no documento, em captura: cobre inclusive os campos
  //     criados depois, sem precisar religar nada a cada render.
  AppCore.protegerNumericos = function () {
    if (AppCore._numProtegidos) return;
    AppCore._numProtegidos = true;
  ...
  // 13. Modal com dados em edição: avisa antes de descartar.
  //
  //     "Começou a preencher" é comparação com o estado de ABERTURA, não
  //     "o campo tem valor": um modal de configuração já abre preenchido com
  //     o que estava salvo, e avisar nesse caso seria alarme falso a cada
  //     abertura — o tipo de aviso que o usuário aprende a ignorar.
  //
  //     A fotografia cobre os controles e também o estado visual das escolhas
  //     (modo selecionado, botão de segmento ligado), porque nesses modais a
  //     escolha do modo é parte do que se perde ao fechar.
  function fotografarModal(raiz) {
    var partes = [];
    ...
    for (var i = 0; i < controles.length; i++) {
      ...
      partes.push((el.id || el.name || 'i' + i) + '=' + v);
    }
    var escolhas = raiz.querySelectorAll('.opt-item, .smb, .seg button, .toggle button');
    for (var j = 0; j < escolhas.length; j++) {
      partes.push('e' + j + '=' + (escolhas[j].className || ''));
    }
    return partes.join('|');
  }

  //     Chamar ao ABRIR o modal.
  AppCore.marcarModalLimpo = function (raiz) {
    var el = typeof raiz === 'string' ? document.getElementById(raiz) : raiz;
    if (el) el.__fotoModal = fotografarModal(el);
  };

  AppCore.modalSujo = function (raiz) {
    var el = typeof raiz === 'string' ? document.getElementById(raiz) : raiz;
    if (!el || el.__fotoModal === undefined) return false;
    return fotografarModal(el) !== el.__fotoModal;
  };

  //     Chamar em TODO caminho de fechamento que descarta (×, Cancelar, Esc,
  //     clique fora). Devolve true quando pode fechar. Quem salva não passa
  //     por aqui: salvar não descarta nada.
  AppCore.podeFecharModal = function (raiz, mensagem) {
    if (!AppCore.modalSujo(raiz)) return true;
    return window.confirm(mensagem ||
      'Você alterou esta configuração e ainda não aplicou.\n\nFechar agora descarta as alterações.');
  };

  window.AppCore = AppCore;

  // Comportamento de base: nenhuma página precisa pedir.
  if (typeof document !== 'undefined') AppCore.protegerNumericos();
})();
